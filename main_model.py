...
class DDPM(nn.Module):
    ...
    def p_sample(self, x, t, clip_denoised=False, condition_x=None):
        ...
        return model_mean + noise * (0.5 * model_log_variance).exp()  # 求指数运算 即e的x次方
    ...
    def p_sample_loop(self, x_in, continous=False):
        ...
        if not self.conditional:
            ...
            for i in reversed(range(0, self.num_steps)):
                ...
                if i % sample_inter == 0:  # 每隔 sample_inter 步对样本进行一次采样
    ...
    def p_losses(self, x_in, y_in, noise=None):
        ...
        def cosine_similarity_loss(clean, x_recon):
            cos = nn.CosineSimilarity(dim=1)
            similarity = cos(clean, x_recon)
            loss = 1 - similarity.mean()
            return loss

        loss = self.loss_func(noise, x_recon)
        loss2 = cosine_similarity_loss(x_in, x_recon)
        return loss * 0.8 + loss2 * 0.2
